...
\kor{\begin{tabular}{r|rrrr}
		...
		\hline
	\end{tabular}
	\caption{Abgelesene Werte von $R_\nu$ in $\left[\Upomega\right]$ für die erkennbaren Hall-Plateaus bei verschiedenen Temperaturwerten mit dem Fehler aus der Ungenauigkeit des Vorwiderstandes von \SI{2}.}}
	\label{klitzing}	
	\end{center}
\end{table}
...
Wie schon zu Beginn der Auswertung bemerkt, ist die Hauptfehlerquelle die geschätzten Ungenauigkeit von \kor{\SI{2}{\percent}} des Vorwiderstandes. \kor{Der Ablesefehler von \SI{20}{\ohm} aus der Abbildung~\ref{hallplateau} kann also vernachlässigt werden.} %Aufgrund der Linearität von Spannung und Widerstand, besitzen die erhaltenen Werte ebenso eine Unsicherheit von \kor{\SI{2}{\percent}}. 
...
\kor{
An den linearen Teil der Messdaten von \SI{0}{\tesla} \--- \SI{1}{\tesla} wurde eine Ursprungsgerade gefittet, wobei sich die Fehlergeraden aus dem Fehler in der Widerstandsmessung von \SI{2}{\percent} ergeben. Zur besseren Darstellung wird in Abbildung~\ref{hallsteigung} nur der Bereich von $0\---\SI{1.3}{\tesla}$ gezeigt.
}
...
\begin{table}[htp]
	\begin{center}
		\begin{tabular}{ccccc}
			...
			\hline
			4.2 & \kor{3.584(70)} & \kor{3.45(12)} & \kor{3.69(17)} & \kor{3.567(61)}\\
			3.0 & \kor{3.634(71)} & \kor{3.37(16)} & \kor{3.69(17)} & \kor{3.583(66)}\\
			2.1 & \kor{3.637(71)} & \kor{3.41(14)} & \kor{3.69(17)} & \kor{3.589(64)}\\
			1.5 & \kor{3.636(71)} & \kor{3.42(15)} & \kor{3.69(17)} & \kor{3.594(65)}\\
			\hline
			\hline
			 &  &  &  &3.583(64)\\
		\end{tabular}
