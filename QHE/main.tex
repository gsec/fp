...
Die Periodendauer $T_{\frac{1}{B}}$ entspricht dabei nach \eqref{periodizitaet} der Periodizität der ganzen Zahle mit dem Faktor $\frac{e}{N_S h}$ multipliziert.
Dadurch wird der Wert für die Ladungsträgerdichte festgelegt.
\begin{equation}
N_S=\frac{e}{T_{\frac{1}{B}} h}=(1.756 \pm 0.040) \cdot 10^{-11}\SI{}{\per\square\centi\meter}
\label{ns_periode}
\end{equation}
Auch diese Methode wurde auf alle Messdaten der unterschiedlichen Temperaturen angewandt. Da die Fouriertransformation auf diskreten Werten geschieht, kann die Periodizität nur diskrete Werte annehmen. Bei den unterschiedlichen Temperaturen wurde jeweils der der selbe Wert ermittelt und zur Übersicht in Tabelle \ref{nswerte} eingetragen.
