...
\centerline{Betreuer: Dr. Charles Gould \hfill
   Versuchsdurchführung am 18. September 2013}
\centerline{\hfill  Protokollabgabe am 25. September 2013}
\centerline{\hfill  Zweitabgabe am 07. Oktober 2013}
...
\begin{figure}[htp]
	\begin{center}
		...
		\caption{Schematischer Versuchsaufbau nach~\kor{Praktikumsanleitung  \citep{anleitung}}.}
...
\begin{figure}[htp]
	...
	\caption{Beschaltung des gesamten Messaufbaus. Auszüge aus~\kor{Praktikumsanleitung  \citep{anleitung}}.}
...
Um die \kor{Abweichung der eingestellten Ausgangsspannung des Funktionsgenerators bei Belastung} zu bestimmen, wird dieser mit dem Oszillographen verbunden. Mit den eingestellten Werten von \SI{50}{\milli\volt} bzw. \SI{100}{\milli\volt} \kor{für die Spitze/Spitze Werte} und \SI{1.3}{\kilo\hertz} bzw. \SI{2.6}{\kilo\hertz} in der Frequenz des Funktionsgenerators messen wir das Cosinus-Signal über den Oszillator, der für die Aufnahme mit dem Computer verbunden ist. An die erhaltenen Messsignale fitten wir jeweils eine Cosinus-Funktion um die Schwankung, die durch die diskrete Zeitauflösung zustande kommt, auszugleichen. Der Fehler der Werte ergibt sich aus der Ungenauigkeit in \kor{den Spitze/Spitze Werten $U_{\rm FG,PP}$} des Funktionsgenerators von \SI{2}{\percent}. Der Fehler der Fitfunktion kann gegen den Fehler in \kor{den Spannungswerten} vernachlässigt werden. Für die vier Messungen vergleichen wir jeweils die Spitze/Spitze Werte, die Amplituden \kor{$U_{\rm Oszi}$} und den Effektivwert. Die Werte sind in Tabelle~\ref{tab.funktionsgenerator} aufgelistet.

\begin{table}[htp]
\begin{center}
	\begin{tabular}{c|ccc}
		\hline
		\multicolumn{1}{c|}{FG $\kor{\SI{50}{\milli\volt}_{\rm PP}}$} &\multicolumn{3}{c}{Oszillograph Spannung in \SI{}{\milli\volt}}\\
\footnotesize Frequenz & \footnotesize Amplitude & \footnotesize Spitze/Spitze & \footnotesize Effektivwert\\
		\hline
		 ...
		 \hline
		\multicolumn{1}{c|}{FG $\kor{\SI{100}{\milli\volt}_{\rm PP}}$} && \\
		 \SI{1.3}{\kilo\hertz} & \SI{100.2(20)}{} & \SI{200.4(40)}{} & \SI{70.87(14)}{}\\
...
\kor{Aus dem Vergleich der Werte in Tabelle~\ref{tab.funktionsgenerator} erkennen wir, dass die Spitze/Spitze Werte am Oszillographen im Rahmen der Fehler genau doppelt so groß sind, wie die eingestellten Werte am Funktionsgenerator. Dies liegt daran, dass der Oszillograph einen sehr großen Widerstand im Vergleich zum Funktionsgenerator besitzt, dieser aber auf eine Belastung von \SI{50}{\ohm} eingestellt ist. Zudem erkennen wir, dass sich die Werte für verschiedene Frequenzen nicht unterscheiden. Das ist sehr wichtig, da wir später bei einer eingestellten Spannung über verschiedene Frequenzen messen wollen.}

Die Einstrahlspule im Manipulator besitzt einen vernachlässigbar kleinen Innenwiderstand. Allerdings ist der Spule ein Vorwiderstand von \SI{47}{\ohm} eingebaut, um den Strom zu begrenzen und messbar zu machen~\citep{anleitung}. Aus dem zusätzlichen Ausgangswiderstand des Funktionsgenerator von \SI{50}{\ohm} wird die Amplitude der Ausgangsspannung reduziert~\citep{anleitung}. Um für nachfolgende Messungen einen Eichfaktor $f_{\rm ES}$ zu erhalten haben wir obige Messung für eine Frequenz von \SI{1.3}{\kilo\hertz} und die \kor{Spitze/Spitze Werte} $\kor{U_{\rm PP}} = \SI{10}{\milli\volt}$, \SI{20}{\milli\volt}, \SI{50}{\milli\volt}, \SI{100}{\milli\volt}, \SI{200}{\milli\volt} und \SI{500}{\milli\volt} wiederholt. Hierbei haben wir jeweils eine Messung mit und eine ohne angeschlossener Einstrahlspule aufgenommen. Zur Bestimmung des Eichfaktors $f_{\rm ES}$ haben wir in Abbildung~\ref{fig.eichfaktor} die gemessenen Spannungen mit angeschlossener Spule $U_{\rm ES}$ über die eingestellten Spannungen des Funktionsgenerators $U_{\rm FG}$ aufgetragen.
...
\begin{equation}
	f_{\rm ES} = \frac{U_{\rm ES}}{U_{\rm FG}} = \kor{\SI{0.967(19)}{}}
	\label{eq.eichfaktor}
\end{equation}
...
\begin{equation}
	\kor{f_{\rm ES} = \frac{U_{\rm Oszi}}{U_{\rm FG}} \cdot \frac{R_{\rm ES}}{R_{\rm FG}+R_{\rm ES}} = \SI{0.969(36)}{}}
\end{equation}
Der Fehler ergibt sich aus \kor{den Fehlern der gemessenen Spannungen und} dem geschätzten Fehler des Vorwiderstandes von \SI{2}{\percent}.

Aus dem Vergleich des experimentell bestimmten Wertes mit dem berechneten sehen wir im Rahmen der Fehler eine sehr gute Übereinstimmung. Für nachfolgende Messungen verwenden wir somit den aus unserer Messung erhaltenen Eichfaktor~\eqref{eq.eichfaktor}.
...
\begin{figure}[t!]
	...
	\end{center}
\end{figure}
Damit wird deutlich, dass die Resonanzfrequenz und der zeitliche Abstand $t_{12}$ zwischen zwei Resonanzfällen einen Cosinus-Zusammenhang besitzen. \kor{Die Periodendauer entspricht dabei der doppelten Periodendauer des Wobbelfeldes.}
Wir beobachten, 
...
\begin{figure}[htp]
	\begin{center}
		...
		\caption{Abgelesene Zeitdifferenzen sind in Abhängigkeit der Frequenz aufgetragen und besitzen eine Cosinus-Abhängigkeit \kor{mit einer Periodendauer von $\SI{0.04}{\second}$}.}
...
\end{align}
\kor{Weiterhin untersuchen wir, wie sich die Signalstärke eines Peaks mit dem Abstand zum vorherigen Peak verändert. Die Signalhöhen werden dazu aus den einzelnen Graphen für die verschiedenen Analysatorfrequenzen abgelesen. Da die Signalstärke nicht in jeder Periode gleich ist,  lesen wir aus der Schwankung der Signalstärken über zehn Perioden einen Fehler ab. Die Beträge der Signalstärken werden in \ref{signalstaerken} aufgetragen und zeigen deutlich, dass die Signalstärke zunimmt, je weiter das letzte Signal zurück liegt.

\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=\columnwidth]{Data-Plots/02-Signale.pdf}
		\caption{Abgelesene Signalstärken in Abhängigkeit zur Zeitdifferenz zum vorherigen Peak.}
		\label{signalstaerken}
	\end{center}
\end{figure}

Der Verlauf lässt sich verstehen, wenn man bedenkt, dass sich während dem Resonanzfall die Polarisation ihrem Gleichgewichtszustand annähert. Dadurch schwächt sich die Polarisation und damit die Signalstärke schnell ab, wird aber durch nachfließendes Wasser erneuert.
Während bei kleinen Zeitdifferenzen nur wenig polarisiertes Wasser in die Spule nachfließen konnte und wir ein schwaches Signal messen, sättigt das Signal bei großen Zeitabständen.}
...
\kor{Aus der Verschiebung lässt sich außerdem ableiten, dass das Messsignal des Analysators der eingestellten Frequenz am Manipulator etwa $3$ Sekunden nacheilt. Dies lässt sich zum Teil damit erklären, dass das Wasser eine gewisse Zeit vom Manipulator zum Analysator benötigt.
Jedoch zeigt die in \ref{wg} berechnete Wassergeschwindigkeit, dass diese nicht alleine für den Verzug verantwortlich sein kann. Was im Messaufbau noch zu einem Verzug führen kann, konnten wir nicht klären.}
...
\kor{Dieses Modell beschreibt das Verhalten unseres Experimentes nicht exakt, da sich das Wasser nicht auf einer geraden Linie aus dem unendlichen durch unsere Leiterschleife bewegt. Die Genauigkeit des Modells lässt sich abschätzen, wenn man nur bis zu einem endlichen Wert integriert und die Resultate vergleicht. Wir verkürzen die Strecke der Protonenbewegung im Modell auf eine Spulenlänge vor und nach der Spule und erhalten eine Abweichung von $\SI{2.3}{\percent}$. Im Rahmen dieses Fehlers können wir den Gesamtdrehwinkel bei einer Einstrahlamplitude von $\SI{45}{\milli\volt}$ berechnen:}
\begin{align}
...
\kor{
Da wir die Messung sowohl von hohen zu niedrigen, als auch von niedrigen zu hohen Intensitäten durchgeführt haben, können wir die Verschiebung der Datenpunkte wie in \ref{auswertung5} aufheben. Außerdem messen wir am Anfang der Messung ein Artefakt, das vermutlich dadurch entstanden ist, dass die Einstrahlspule kurz vor der Messung bereits aktiv war. Wir entscheiden uns daher hier den Sweep von hohen zu niedrigen Einstrahlintensitäten zu plotten und finden deshalb das Artefakt bei hohen Intensitäten.
Die symmetrisierten Messwerte sind in \cref{symmetr} zusammen mit einer theoretisch erwarteten Kurve gezeichnet. Die theoretische Kurve beschreibt die Funktion \eqref{theospin} im Resonanzfall mit den Parametern $c=0.27$ und dem Parameter $p=70$, der als Proportionalitätskonstante zwischen der Einstrahlintensität und dem Spindrehwinkel den Einfluss der Wassergeschwindigkeit und der Spulengeometrie erfasst.

\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=\columnwidth]{Data-Plots/11-Spindrehkurve.pdf}
		\caption{Symmetrisierte Messpunkte im Vergleich mit einer theoretisch erwarteten Spinkurve.}
		\label{symmetr}
	\end{center}
\end{figure}

Die Theorie beschreibt unser Experiment mit den verwendeten Parametern nur qualitativ und wir finden kein anderes Parameterset, dass unser Experiment genauer beschreibt.
Die Theorie Berücksichtigt nach \citep{anleitung} unterschiedliche Wassergeschwindigkeiten in der Manipulatorspule was den Abfall der Intensitäten bei größeren Drehwinkeln erklärt.
Auffällig ist eine sich ändernde Periodendauer der Oszillation der experimentellen Daten. 
Dies wird vom theoretischen Modell nicht erfasst, sondern kann theoretisch nur erklärt werden, wenn sich während des Durchfahrens der Einstrahlintensität das Verhältnis zwischen Resonanzfrequenz und Einstrahlfrequenz verändert.
}
...
\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=\columnwidth]{Data-Plots/13-Halbwertsfrequenz180.pdf}
		\caption{Schnitt durch Phasenraum mit Einstrahlintensität $\SI{45}{\milli\volt}$.}
...
\end{align}
\kor{
Der theoretisch erwartete Verlauf wurde für die Resonanzfrequenz $\nu_\mathrm{res}=\SI{1510}{\hertz}$ und einer einstrahlenden Magnetfeldstärke von $\SI{1.1}{\micro\tesla}$ gefittet.
Abweichungen des Experiments von der theoretisch erwarteten Kurve sind zum einen, dass der Peak der Resonanzfrequenz im Experiment bei gleicher Höhe eine geringere Halbwertsbreite besitzt. Außerdem fehlen die von der Theorie vorhergesagten Oszillationen für weiter abweichende Frequenzen ganz.}
...
\kor{Für den Fall einer  $360$\textdegree -Drehung sagt die Theorie ein Minimum des Signals bei der Resonanzfrequenz voraus. Wir messen aber bei allen Einstrahlintensitäten ein maximales Signal bei der Resonanzfrequenz. Die theoretische Kurve wurde mit den gleichen Parametern wie in \ref{halbwertsbreite} berechnet.
Die theoretischen und experimentellen Daten hierzu in \ref{halbwertsbreite2} geplottet.}


\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=\columnwidth]{Data-Plots/13-Halbwertsfrequenz360.pdf}
		\caption{Schnitt durch Phasenraum  mit Einstrahlintensität $\SI{90}{\milli\volt}$.}
		\label{halbwertsbreite2}
	\end{center}
\end{figure}
...
Das Feld im Zentrum der beiden Spulen kann mit der Näherung aus~\kor{Praktikumsanleitung  \citep{anleitung}} bestimmt werden.
\begin{equation}
...
\end{equation}
Für die Resonanzfrequenzen im Magnetfeld gilt nach~\kor{Praktikumsanleitung  \citep{anleitung}}
\begin{equation}
...
\begin{table}[htp]
	...
	\begin{tabular}{cc}
		...
		$I_{\rm HH}[\mathrm{mA}]$ & $\nu_{\rm res}[\mathrm{Hz}]$\\
		\hline	
		$+ 45.0 \pm \kor{0.9}$   & $ 6826.3 \pm 2.0$\\
		$- 45.0 \pm \kor{0.9}$   & $ 5562.2 \pm 2.0$\\
		$+ 65.0 \pm \kor{1.3}$   & $ 9480.2 \pm 2.0$\\
		$- 65.0 \pm \kor{1.3}$   & $ 8207.7 \pm 2.0$\\
		$+ 80.0 \pm \kor{1.6}$   & $11473.2 \pm 2.0$\\
		$- 80.0 \pm \kor{1.6}$   & $10195.6 \pm 2.0$\\
		\hline
	\end{tabular}	
...
Der Ungenauigkeit in der Einstellung des Helmholtzspulenstroms betrug \SI{0.2}{\milli\ampere}. \kor{Dieser Fehler wird allerdings vom Fehler des Strommessgerätes von etwa \SI{2}{\percent} überlagert} und ergibt den Fehler in $I_{\rm HH}$. Der Fehler der Resonanzfrequenz entspricht der Ablesegenauigkeit von \SI{2}{\hertz}.
...
\kor{\begin{align}
	\mu_{\rm K} &= \SI{5.10(20)e-27}{\tesla\per\ampere}\\
	B_{\rm S} &=  \SI{34(11)}{\micro\tesla}
\end{align}}

Der Literaturwert für das Kernmagneton beträgt nach dem~\kor{\citet{codata}}
\begin{equation}
	\mu_{\rm K,Lit} = \SI{5.05078e-27}{\tesla\per\ampere}.
\end{equation}

Der experimentell ermittelte Wert ist im Rahmen seiner Fehler mit dem Literaturwert vereinbar. Das Ergebnis für den Betrag des Störfeldes $B_{\rm S}$ liegt im Bereich der Stärke des Erdmagnetfeldes, was wir auch erwartet hatten. Den Wert des Störfeldes im Raum haben wir zudem in Versuchsteil~\ref{auswertung5} durch die Bestimmung der Resonanzfrequenz bestimmt. \kor{Der hier erhaltenen Fehler für das Störfeld ist aufgrund der ungenauen Strommessung sehr groß, sodass die beiden erhaltenen Werte im Rahmen ihrer Fehler gut miteinander vereinbar sind}. %Dies liegt an systematischen Fehlerquellen in der Messung. So ist es möglich, dass die Resonanzfrequenz nicht exakt eingestellt war oder der Spulenstrom in den beiden Messungen für positive und negative Richtung doch nicht exakt gleich war.

Um nun noch den Winkel  des Störfeldes zu bestimmen, verwenden wir die Differenz der Frequenzquadrate nach~\kor{Praktikumsanleitung  \citep{anleitung}},
\begin{equation}
...
Aus Abbildung~\ref{fig.stoerwinkel} erhalten wir für die Steigung der Ausgleichsgeraden \kor{$m_{\rm Diff} = \SI[per-mode=symbol]{344(25)e3}{\hertz\squared\per\milli\ampere}$} mit dem Fehler durch die Geraden minimaler und maximaler Steigung.
Mit Gleichung~\eqref{eq.stoerwinkel} erhalten wir somit den Winkel des Störfeldes.
\kor{\begin{equation}
	\phi = \SI{64.7(85)}{\degree}
\end{equation}}

Der Fehler im Winkel $\phi$ folgt nach Gauß'scher Fehlerfortpflanzung aus den Fehlern für das Störfeld $B_\mathrm{S}$, der ermittelten Steigung $m_{\rm Diff}$ und dem Kernmagneton $\mu_{\rm K}$. Das Störfeld im unseren Experiment ist also mit einem Winkel \kor{$\phi = \SI{64.7(85)}{\degree}$} relativ zum Feld der Helmholtzspulen ausgerichtet. Wir vermuten, dass das Feld in Richtung Zimmerdecke (oder entgegengesetzt) zeigt. Dies liegt daran, dass wir den Polarisator als Hauptstörquelle ausmachen und dieser in dieser Richtung im Raum steht.
...
Da die Temperatur im Versuch nicht konstant gehalten konnte, haben wir bei jeder Messung die Temperatur mit einem batteriebetriebenen Thermometer gemessen und für die gemeinsame Auftragung bei verschieden Wassergeschwindigkeiten gemittelt. \kor{Der Fehler für den Temperaturwert bei etwa \SI{20}{\celsius} ist deutlich kleiner, da dies der Arbeitstemperatur entspricht, die während der Messreihe nahezu unverändert bleibt.} Die Fehler in Abbildung~\ref{fig.relaxation} resultieren bei der Zeitdifferenz aus einem Ablesefehler von \SI{0.05}{\second} und in der Signalhöhe aus einem Ablesefehler von \SI{0.02}{\volt} und der Ungenauigkeit in S/H-Signal von \SI{5}{\percent}.
Die Messung ist zudem nur aussagekräftig, wenn Strömungen turbulent sind. Die minimale Fließgeschwindigkeit kann mit einer Reynoldszahl von 3000 bestimmt werden. Bei über $R = 3000$ spricht man von turbulenter und unter $R=2000$ von laminarer Strömung. Mit folgender Formel aus~\kor{Praktikumsanleitung  \citep{anleitung}}
\begin{equation}
	R=\frac{2\rho rv}{\eta}
\end{equation}
...
An die Messdaten aus Abbildung~\ref{fig.relaxation} wurden Exponentialfunktionen gefittet und mit $\mathrm{d}M_{z}(t) = -1/T_1 M_{z}(t)\mathrm{d}t$ nach~\kor{Praktikumsanleitung  \citep{anleitung}} die Relaxationszeit $T_1$ bestimmt. Die erhaltenen Werte sind in Tabelle~\ref{tab.relaxationszeiten} aufgetragen.

\begin{table}
	\begin{center}
\kor{\begin{tabular}{cc}
		\hline
		$T[\SI{}{\celsius}]$ & $T_1[1/\mathrm{s}]$\\
		\hline
		\SI{14(1)}{\celsius} & $3.6^{+1.5}_{-1.9}$\\ %3.647301884	1.701329412
		\SI{34.0(2)}{\celsius} & $4.6^{+1.6}_{-1.5}$\\ %4.606282289	1.51535674
		\SI{42.5(7)}{\celsius} & $5.5^{+1.9}_{-1.9}$\\ %5.487573373	1.937461341
		\SI{63(2)}{\celsius}  & $9.8^{+6.1}_{-6.1}$\\ %9.814701108	6.141836515
		\hline
	\end{tabular}}
...
\end{equation}
Das hier erhaltenen Ergebnis wird für die Berechnungen in Versuchsteil~\ref{auswertung5} verwendet.
...
Die Drehwinkel der Magnetisierung im Manipulator wurden den Einstrahlamplitude eindeutig zugeordnet~\eqref{auswertung5}.
 ...
 Mit Hilfe eines Helmholtzspulenpaares konnten wir verschieden starke statische Magnetfelder bereitstellen. Aus der Untersuchung der entsprechenden Resonanzfrequenzen konnten wir das im Raum vorhandenen Störfeld $\vec{B}_{\rm S}$ berechnen. Die Stärke des Störfeldes betrug $B_{\rm S} = \SI{34(11)}{\micro\tesla}$ und hatte einen Winkel von $\phi = \SI{64.7(85)}{\degree}$~\eqref{auswertung6} zum Helmholtzspulenfeld. 
Der Betrag des Störfeldes stimmt dabei gut mit der bestimmten Resonanzfrequenz aus den Frequenz-Sweeps überein~\eqref{auswertung5}. Den Wert für das Kernmagneton konnten wir mit $\mu_{\rm K} = \SI[separate-uncertainty=false, per-mode=symbol]{5.10(20)e-27}{\tesla\per\ampere}$ in Übereinstimmung mit dem Literaturwert bestimmen~\eqref{auswertung6}.
Mit einer detaillierten Analyse des Signals bei verschiedenen Wassergeschwindigkeiten konnten wir die Wassergeschwindigkeit bei maximaler Pumpenleistung auf $\SI[per-mode=symbol]{3.10(24)}{\meter\per\second}$ in der Manipulatorspule bestimmen. Daraus errechneten wir die Relaxationszeit $T_1$ der Protonen bei Raumtemperatur zu \kor{$4.6^{+1.6}_{-1.5}\,\SI{}{1\per\second}$} ~\eqref{auswertung7}. Die Untersuchung der Relaxationszeit für verschiedene Wassertemperaturen zeigt ein uns unerwartetes Ergebnis, dass die Protonen bei höherer Wassertemperatur langsamer relaxieren. Insgesamt konnten wir die Magnetisierung und deren Manipulation  untersuchen und die vorhergesagt Dynamik zu großen Teilen bestätigen. 
