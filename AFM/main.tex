...
this in not the case however. During the measurement of sample \textcircled{?} 
...
as they only appear in one, either forward or backward direction. The first 
artifact we want to discuss is shown in \cref{artifact1}. In one single line we 
can see a periodic pattern with the periodicity of approximately 
$\SI{2.5}{\micro\meter}$. But this can  be found with an other periodicity as 
well.
...
tube or other elements. Implemented in the design of the AFM every scan is 
